...
const UpdateAdminMed = ({ medicine, closeModal, refetch }) => {
    const { user } = useAuth()
    ...
    const onSubmit = async (data) => {
        if (role !== "admin") {
            Swal.fire("Access Denied", "Only admins can update medicines.", "error");
            return;
        }
        ...
        try {
            ...
            const res = await axios.patch(
                `/admin/medicine/update/${medicine._id}?email=${user.email}`,
                updatedData
            );
